...
usage() {
    ...
    echo "--plotStepOnly <only plot the PDFs, skip previous steps>"
    #TODO make a dryrun option
...
PARSED=$(getopt -o o:f:p:h --long output:,fittedPDF:,pdPDF:,plotStepOnly,help -- "$@")
if [[ $? -ne 0 ]]; then
...
#fit_result=$(echo "$fit_result" | sed 's/\x1B\[[0-9;]*[a-zA-Z]//g') # sanitize the output
echo "$fit_result"

echo
echo "make_postfit_pdf_grids"

if ! $plotStepOnly; then python3 ${WREM_BASE}/scripts/utilities/make_postfit_pdf_grids.py --fitresult $fit_result -o $output_dir --pseudoData nominal_gen_pdf${pdPDF^^}_pdfVar -l $postfix_arg
fi
...
compound_label_value="'${fittedPDF^^} (gen fit ${pdPDF^^})'"

python3 ${WREM_BASE}/scripts/plotting/plot_pdfs.py --lhapdf-path $output_dir -s NNPDF40_nnlo_hessian_pdfas MSHT20nnlo_as118 NNPDF40_nnlo_hessian_pdfas_nnpdf40_pdMSHT20_unscaled -o $output_plot_dir -l ${fittedPDF^^} ${pdPDF^^} $compound_label_value -f dv uv 1 2 3 --colors r g b
#echo "$plot_command"
#plot_output=$($plot_command 2>&1 | tee /dev/tty)
